...
export default function AlunoDashboard() {
  ...
  useEffect(() => {
    Promise.all([
      api.get("/me/perfil"),
      api.get("/me/mensalidades"),
      api.get("/me/graduacoes"),
    ]).then(([p, m, g]) => {
      setPerfil(p.data.dados);
      ...
      <Row className="g-3 mb-4">
        <Col xs={12} sm={4}>
          <div style={{ background: "var(--color-background-secondary)", borderRadius: "var(--border-radius-md)", padding: "1rem", textAlign: "center" }}>
            ...
            <div style={{ fontSize: "18px", fontWeight: 500, marginTop: 4 }}>{faixaAtual}</div>
            <div style={{ fontSize: "12px", color: "var(--color-text-secondary)" }}>faixa atual</div>
          </div>
        </Col>
        <Col xs={12} sm={4}>
          <div style={{ background: pendentes.length > 0 ? "var(--color-background-danger)" : "var(--color-background-success)", borderRadius: "var(--border-radius-md)", padding: "1rem", textAlign: "center" }}>
            ...
            <div style={{ fontSize: "20px", fontWeight: 500, color: pendentes.length > 0 ? "var(--color-text-danger)" : "var(--color-text-success)", marginTop: 4 }}>{pendentes.length}</div>
            <div style={{ fontSize: "12px", color: "var(--color-text-secondary)" }}>mensalidade(s) pendente(s)</div>
          </div>
        </Col>
        <Col xs={12} sm={4}>
          <div style={{ background: "var(--color-background-secondary)", borderRadius: "var(--border-radius-md)", padding: "1rem", textAlign: "center" }}>
            ...
            <div style={{ fontSize: "12px", color: "var(--color-text-secondary)" }}>graduação(ões)</div>
          </div>
        </Col>
      </Row>
      {pendentes.length > 0 && (
        <Alert variant="warning">Você possui <strong>{pendentes.length}</strong> mensalidade(s) pendente(s). <a href="/aluno/minhas-mensalidades">Ver detalhes</a></Alert>
      )}
      <Row className="g-3">
        {[
