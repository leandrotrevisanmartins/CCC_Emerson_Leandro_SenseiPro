...
export const turmaService = {
  ...
  create: (data: Partial<TurmaInterface> & { id_modalidade: number; id_professor: number }) =>
    api.post<TurmaInterface>("/turmas", data).then(r => r.data),
  ...
  matricular: (id_turma: number, id_aluno: number) =>
    api.post(`/turmas/${id_turma}/matricular`, { id_aluno }).then(r => r.data),
  desmatricular: (id_turma: number, id_aluno: number) =>
    api.delete(`/turmas/${id_turma}/aluno/${id_aluno}`).then(r => r.data),
};
...
export const presencaService = {
  ...
  create: (data: { id_aluno: number; id_turma: number; data: string; presente: boolean }) =>
    api.post<PresencaInterface>("/presencas", data).then(r => r.data),
  update: (id: number, data: Partial<PresencaInterface>) => api.put<PresencaInterface>(`/presencas/${id}`, data).then(r => r.data),
  delete: (id: number) => api.delete(`/presencas/${id}`),
};
...
export const mensalidadeService = {
  ...
  create: (data: { id_aluno: number; mes_referencia: string; valor: number; data_vencimento: string }) =>
    api.post<MensalidadeInterface>("/mensalidades", data).then(r => r.data),
  update: (id: number, data: Partial<MensalidadeInterface>) => api.put<MensalidadeInterface>(`/mensalidades/${id}`, data).then(r => r.data),
  delete: (id: number) => api.delete(`/mensalidades/${id}`),
};

// ─── Pagamentos ───────────────────────────────────────────────────
export const pagamentoService = {
  getByMensalidade: (id: number) => api.get<PagamentoInterface[]>(`/pagamentos/mensalidade/${id}`).then(r => r.data),
  create: (data: { id_mensalidade: number; valor_pago: number; forma_pagamento: string }) =>
    api.post<PagamentoInterface>("/pagamentos", data).then(r => r.data),
...
export const graduacaoService = {
  ...
  create: (data: { id_aluno: number; faixa: string; data_graduacao: string; observacao?: string }) =>
    api.post<GraduacaoInterface>("/graduacoes", data).then(r => r.data),
  update: (id: number, data: Partial<GraduacaoInterface>) => api.put<GraduacaoInterface>(`/graduacoes/${id}`, data).then(r => r.data),
  delete: (id: number) => api.delete(`/graduacoes/${id}`),
};
